...
posts.forEach((post: any) => {
  ...
  feed.addItem({
    ...
    content: he.decode(
      `<p><img width="1000" style="max-width: 100%; height: auto; object-fit: contain; display: block; margin: 0 auto;" src="${post._embedded?.["wp:featuredmedia"]?.[0]?.source_url || ""}" alt="${he.decode(post.title.rendered)}" decoding="async" /></p>` + post.content.rendered
    ),      
